...
function displayMovies(movies) {
  ...
  movies.forEach((movie, idx) => {
    ...
    card.innerHTML = `
      ...
      <h3><a href="${movie.imdb_link || movie.imdb || '#'}" target="_blank">${movie.title}</a></h3>
...
function getRecommendations() {
  ...
  fetch(`${backendUrl}/recommend`, {
    ...
    body: JSON.stringify({
      ...
      newSearch: true  // Important: reset session data on every new search
    })
  })
    ...
    .then(data => {
      if (data.recommendations) {
        ...
        feedback = {};  // clear feedback on new search
